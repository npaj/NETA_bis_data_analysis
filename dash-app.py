...
def Update_map(click,current_time, winsize, cmin2, cmax2, th):
    ...
    for idx in tqdm(range(len(Df_data))):
        ...
        X = np.abs(np.fft.fft(x*sig.windows.hann(len(x), False), nfft))[np.logical_and(freq> float(cmin2), freq < float(cmax2))]
        freq = freq[np.logical_and(freq> float(cmin2), freq < float(cmax2))]
